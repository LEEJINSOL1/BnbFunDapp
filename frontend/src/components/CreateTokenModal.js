...
const TOKEN_FACTORY_ADDRESS = "0x7E5b012c635d9bB9c19aAD3E37F059C4410E3AA1";
...
const CreateTokenModal = ({ onClose, onCreate, signer }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requiredBalance = ethers.parseEther("0.013");
      if (balance < requiredBalance) {
        setError("Insufficient BNB balance. Need at least 0.013 BNB.");
        return;
      }
      ...
      const event = receipt.logs.find((log) => log.topics[0] === ethers.id("TokenCreated(address,address)"));
      const [tokenAddress, presaleAddress] = ethers.AbiCoder.defaultAbiCoder().decode(
        ["address", "address"],
        event.data
      );
  
      const token = { name: name.trim(), symbol: symbol.trim().toUpperCase(), image, tokenAddress, presaleAddress };
